...
- **Which cost centre dominates is a property of the product, not of the process** — added 2026-08-13 by the `D23` pass for #87, and it is the bullet that carries the decision. Downstream processing is reported at **45–92%** of production cost for biopharmaceuticals against **20–40%** for bulk fermentation products, where raw material takes the larger share instead (Straathof 2011). <!-- ref: 2011-straathof-downstream-costs --> So the *weighting* between titer's cost centre and yield's moves by product class. A single-metric objective cannot track a weighting that moves; net $/kg can. This also reconciles the two sources this decision now cites: Konzock & Nielsen's ">50% substrate for commodity products" and Straathof's "DSP dominates for biopharmaceuticals" are not in tension — they are opposite ends of one axis.

  > **Provenance, stated because this decision has been burned once already.** Straathof's chapter is paywalled and these percentages come from its abstract, quoted consistently by several independent sources, rather than from the chapter's own tables. That is materially better than the industry-blog provenance behind the withdrawn 35–50% split, and it is still secondary; the register carries the row as `partially supports`. What does not depend on the exact figures — and what the argument actually rests on — is the qualitative shape: DSP dominates for high-value low-titer products and does not for bulk ones.
