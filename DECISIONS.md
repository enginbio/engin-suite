...
**Amended 2026-08-25 (#242), on the three points the correction above left open: the cost, the terminus, and what "visibility push" bundles.** The decision is not reversed — the asymmetry argument stands and broadcast stays gated.

*Cost.* "A few months of obscurity" understates it, and the understatement is load-bearing because it is what makes waiting look cheap. NSF's PESOSE solicitation (NSF 26-506, §V.A) requires, on **every** track including the $300k scoping track: *"A minimum of three and up to five letters of collaboration from third-party users and/or contributors of the open-source product… who are not directly related to the proposing team."*
<!-- ref: 2026-nsf-pesose-solicitation --> Engin has none, so the 2026-09 cycle is unreachable and 2027-03 is reachable only with roughly six months of adoption work in front of it. `D25`'s route needs the same population and `D18`'s maintainer-promotion trigger is the same person. **The cost of this ordering includes at least one grant cycle.** Verified against the solicitation itself, not through the issue that found it.

*Terminus.* The bar is **the two conditions as written, and they are met.** Findings from the standing scan are inputs to the ordering below, not a fourth condition. The scan runs daily and will always produce something, so "no open findings" would make this decision permanent by construction — a ratchet rather than a gate, which is not what it was written to be.

**What "visibility push" bundles — three things, split by reversibility.**

1. **Contact** — joining communities, answering questions, approaching named individuals, publishing honestly. **Never gated by this decision**, stated here because the bundling has been read as though it were. This tier is what the PESOSE dependency actually needs: three unrelated users is a contact problem, not an audience problem, and the old bundling forbade solving it.
2. **On-ramps** — documentation and artifacts that let someone who arrives on their own contribute without asking. **Not gated.** Building a door is not standing in the street.
3. **Broadcast** — a podcast appearance, an announcement to a community list, a front-page post. One-shot and non-repeatable, and the only tier the asymmetry argument is about. **Gated.**

**The distinction is reversibility, not convenience, and that is the load-bearing claim.** A conversation with one person can be repeated later with better material; an announcement to three thousand cannot. This decision's own "why a decision and not just a plan" paragraph warns that a project with no users can always tell itself that talking to people *is* the validation — that warning still applies to tiers 1 and 2, and the test of this amendment is whether they stay contact and on-ramps. **If tier 1 becomes a launch, this amendment has failed and should be reversed.**

*Discharge for tier 3.* #242 proposed "#190 resolved or the claim withdrawn", and that was declined on the evidence. #190 is real — `simulator._rhs` returns `[dX, dS, dP, dV]` with no oxygen state — but it is **already disclosed on the front page** (`docs/index.md`), so the credibility trap the proposal described does not exist. The actual defect was narrower: `engin_core.tea` genuinely models plant capital and its recovery, so "scale-up economics" was not false, but it invited a reader to think the *biology* forecast transfers to a new scale when the one thing that decides that — oxygen transfer — is absent. **The pitch now reads "production-scale costing", and says in the same breath that the costing scales and the fermentation model does not.** That discharges tier 3.

**What this does not license.** It is an ordering decision, not a marketing plan. Nothing here says a broadcast should happen, only that this decision has stopped forbidding one. `D12` still gates the claims, and a claim that has not been validated on real data does not become sayable because the audience question was settled.
